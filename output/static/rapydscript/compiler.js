...
Dictionary.prototype = {
    ...
    each: function(f) {
        var i;
        var _$rapyd$_Iter4 = dir(this._values);
    ...
    map: function(f) {
        var ret, i;
        ret = [];
        var _$rapyd$_Iter5 = dir(this._values);
...
function DEFNODE(type, props, methods, base) {
    ...
    if (methods) {
        var _$rapyd$_Iter6 = dir(methods);
    ...
    function display_complex_body(node, is_toplevel, output) {
        ...
        if (node instanceof AST_Scope) {
            if (node.argnames) {
                ...
                var _$rapyd$_Iter9 = dir(node.argnames.defaults);
                for (var _$rapyd$_Index9 = 0; _$rapyd$_Index9 < _$rapyd$_Iter9.length; _$rapyd$_Index9++) {
                    arg = _$rapyd$_Iter9[_$rapyd$_Index9];
                    output.indent();
                    output.print("if");
                    output.space();
                    output.with_parens(function() {
...
function parse($TEXT, options) {
    ...
    function_ = function(in_class, ctor) {
        ...
        nonlocals.forEach(function(variable) {
            var i;
            var _$rapyd$_Iter15 = dir(definition.localvars).reverse();
    ...
    function expr_list(closing, allow_trailing_comma, allow_empty, func_call) {
        ...
        if (func_call) {
            tmp = [];
            tmp.kwargs = [];
            var _$rapyd$_Iter16 = enumerate(a);
...
compile = function(src) {
    var rs_options, output_opts, output, TOPLEVEL, result;
    rs_options = {};
    output_opts = {
        "beautify": true,
        "private_scope": false,
